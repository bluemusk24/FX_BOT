...
class TrainModel:
  ...
  def _define_feature_sets(self):
    ...
    self.MACRO = ['gdppot_us_yoy', 'gdppot_us_qoq', 'cpi_core_yoy', 'cpi_core_mom', 'FEDFUNDS', 'trade_balance_us_yoy','trade_balance_us_mom',
        'DGS1', 'DGS5', 'DGS10']
  ...
  def _define_dummies(self):
      ...
      self.transformed_df.loc[:,'Month'] = pd.to_datetime(self.transformed_df.Month_x, errors='coerce').dt.strftime('%B')
      self.transformed_df['Weekday'] = self.transformed_df['Weekday'].astype(str)
      self.transformed_df['Month_Weekday'] = self.transformed_df['Month'] + '_' + self.transformed_df['Weekday']
  ...
  def train_xgboost(self, **kwargs):
    print('Training the best XGBClassifier model with the best tuned parameters')
    self.model = xgb.XGBClassifier(
       colsample_bytree=0.9479019677815951, eval_metric='auc', 
       gamma= 0.05008900794612733, learning_rate=0.007823913404021691, 
       max_depth=8, min_child_weight=25.589266456955176, 
       n_estimators=200, objective='binary:logistic', 
       random_state=42, reg_alpha=0.03222718425245899, 
       reg_lambda=0.5596981230656976, subsample=0.32280088746826135
    )
  ...
  def make_inference(self, pred_name:str):
    ...
    y_pred_all = self.model.predict_proba(self.X_all)[:,1] #list of predictions for class "1"
    print("Sample predicted probabilities:", y_pred_all[:20])
    y_pred_all_class1 = y_pred_all >= 0.56
    y_pred_all_class1_array = np.array(y_pred_all_class1)
    print("Buy signals (True):", np.sum(y_pred_all_class1_array))
    print("Sell signals (False):", np.sum(~y_pred_all_class1_array))

    self.df_full[pred_name] = y_pred_all_class1_array
    self.df_full[f"prob_{pred_name}"] = y_pred_all
    ...
    # Save to local_data/tickers_df.parquet for Streamlit app
