...
class TransformData:
  ...
  def transform(self):
    ...
    self.transformed_df = self.transformed_df[self.transformed_df.Date>='2023-08-27']
  ...
  def _transform_tickers(self):
    ...
    for ticker in tickers:
     ...
     # merge to one df
      m1 = pd.merge(current_ticker_df, df_current_ticker_momentum_indicators, how = 'left', on = ["Date","Ticker"], validate = "many_to_one")
      m2 = pd.merge(m1, df_current_ticker_volume_indicators, how = 'left', on = ["Date","Ticker"], validate = "many_to_one")
      m3 = pd.merge(m2, df_current_ticker_pattern_indicators, how = 'left', on = ["Date","Ticker"], validate = "many_to_one")
